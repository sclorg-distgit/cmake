...
%{?scl:%scl_package %{orig_name}%{?name_suffix}}
...
Name:           %{?scl_prefix}%{orig_name}%{?name_suffix}
...
Source1:        %{pkg_name}-init.el
Source2:        macros.%{pkg_name}
# See https://bugzilla.redhat.com/show_bug.cgi?id=1202899
Source3:        %{pkg_name}.attr
Source4:        %{pkg_name}.prov
...
Patch2:         %{pkg_name}-findruby.patch
# replace release flag -O3 with -O2 for fedora
Patch3:         %{pkg_name}-fedora-flag_release.patch
# add extra aarch32 to libarch for arm platform
Patch4:         %{pkg_name}-libarch-arm-findjni.patch
...
Patch5:         %{pkg_name}-3.6.1-riscv.patch

# Patch for renaming on EPEL
%if 0%{?name_suffix:1}
Patch1000:      %{pkg_name}-rename.patch
%if 0%{?rhel} && 0%{?rhel} <= 6
Patch1001:      %{pkg_name}-libarchive3.patch
%endif
%endif
...
BuildRequires: llvm-toolset-7-build

Requires:       %{name}-data = %{version}-%{release}
Requires:       rpm

# Provide the major version name
Provides: %{?scl_prefix}%{orig_name}%{major_version} = %{version}-%{release}
...
%endif
tail -n +2 %{SOURCE4} >> %{pkg_name}.prov
...
mkdir build
pushd build
../bootstrap --prefix=%{_prefix} --datadir=/share/%{pkg_name} \
             --docdir=/share/doc/%{pkg_name} --mandir=/share/man \
...
find %{buildroot}/%{_datadir}/%{pkg_name}/Modules -type f | xargs chmod -x
[ -n "$(find %{buildroot}/%{_datadir}/%{pkg_name}/Modules -name \*.orig)" ] &&
  echo "Found .orig files in %{_datadir}/%{pkg_name}/Modules, rebase patches" &&
  exit 1
popd
...
for f in %{buildroot}%{_datadir}/%{pkg_name}/completions/*
do
  ln -s ../../%{pkg_name}/completions/$(basename $f) %{buildroot}%{_datadir}/bash-completion/completions/
done
# Install emacs cmake mode
mkdir -p %{buildroot}%{?scl:%{_scl_root}}%{_emacs_sitelispdir}/%{pkg_name}
install -p -m 0644 Auxiliary/cmake-mode.el %{buildroot}%{?scl:%{_scl_root}}%{_emacs_sitelispdir}/%{pkg_name}/%{pkg_name}-mode.el
%{_emacs_bytecompile} %{buildroot}%{?scl:%{_scl_root}}%{_emacs_sitelispdir}/%{pkg_name}/%{pkg_name}-mode.el
mkdir -p %{buildroot}%{?scl:%{_scl_root}}%{_emacs_sitestartdir}
install -p -m 0644 %SOURCE1 %{buildroot}%{?scl:%{_scl_root}}%{_emacs_sitestartdir}/
# RPM macros
install -p -m0644 -D %{SOURCE2} %{buildroot}%{rpm_macros_dir}/macros.%{pkg_name}
sed -i -e "s|@@CMAKE_VERSION@@|%{version}|" -e "s|@@CMAKE_MAJOR_VERSION@@|%{major_version}|" %{buildroot}%{rpm_macros_dir}/macros.%{pkg_name}
touch -r %{SOURCE2} %{buildroot}%{rpm_macros_dir}/macros.%{pkg_name}
%if 0%{?_rpmconfigdir:1}
# RPM auto provides
install -p -m0644 -D %{SOURCE3} %{buildroot}%{_prefix}/lib/rpm/fileattrs/%{pkg_name}.attr
install -p -m0755 -D %{pkg_name}.prov %{buildroot}%{_prefix}/lib/rpm/%{pkg_name}.prov
%endif
...
%if 0%{?with_sphinx:1}
mv %{buildroot}%{_docdir}/%{pkg_name}/html .
%endif
rm -rf %{buildroot}%{_docdir}/%{pkg_name}
# Install documentation to _pkgdocdir
mkdir -p %{buildroot}%{_pkgdocdir}
cp -pr %{buildroot}%{_datadir}/%{pkg_name}/Help %{buildroot}%{_pkgdocdir}
...
#bin/ctest%{?name_suffix} -V -E 'CMake.FileDownload|CTestTestUpload|curl' %{?_smp_mflags}
#popd
#%if 0%{?rhel} && 0%{?rhel} <= 6
#mv -f Modules/FindLibArchive.disabled Modules/FindLibArchive.cmake
#%endif

#find %{buildroot} -iname cmake.prov
#find %{buildroot} -iname cmake.prov
...
%doc COPYING*
%{_bindir}/c%{pkg_name}
%{!?name_suffix:%{_bindir}/c%{pkg_name}%{major_version}}
%{_bindir}/%{pkg_name}
%{!?name_suffix:%{_bindir}/%{pkg_name}%{major_version}}
...
%{_datadir}/%{pkg_name}/
%if 0%{?fedora} || 0%{?rhel} >= 7
%{?scl:%{_scl_root}}%{_emacs_sitelispdir}/%{pkg_name}
%{?scl:%{_scl_root}}%{_emacs_sitestartdir}/%{pkg_name}-init.el
%else
%{?scl:%{_scl_root}}%{_emacs_sitelispdir}
%{?scl:%{_scl_root}}%{_emacs_sitestartdir}
%endif
%{rpm_macros_dir}/macros.%{pkg_name}
%if 0%{?_rpmconfigdir:1}
%{_prefix}/lib/rpm/fileattrs/%{pkg_name}.attr
%{_prefix}/lib/rpm/%{pkg_name}.prov
%endif
...
%if 0%{?with_sphinx:1}
%{_mandir}/man1/%{pkg_name}-gui.1.*
%endif
%endif


%changelog
* Thu Jun 08 2017 Tom Stellard <tstellar@redhat.com> - 3.6.2-9
- Fix install path for emacs files

* Fri Jun 02 2017 Tom Stellard <tstellar@redhat.com> - 3.6.2-8
- Rename llvm-toolset-4 -> llvm-toolset-7

* Fri Apr 28 2017 Tom Stellard <tstellar@redhat.com> - 3.6.2-7
- Fix build for llvm-toolset-4

* Thu Apr 13 2017 Omair Majid <omajid@redhat.com> - 3.6.2-6
- SCLize package
- Enable bootstrap
- Disable GUI
